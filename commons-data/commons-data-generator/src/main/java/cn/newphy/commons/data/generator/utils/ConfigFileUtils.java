...
import cn.newphy.commons.data.generator.config.Config;
...
public class ConfigFileUtils {
	private static final Logger logger = LoggerFactory.getLogger(ConfigFileUtils.class);
	 ...
	 public static Config getConfig(String configFile){
		 InputStream is = ReflectUtils.class.getClassLoader().getResourceAsStream(configFile);
		...
		try {
			...
			return null;//JSONObject.parseObject(JSONObject.toJSONString(map),Config.class);
		} catch (Exception e) {
			logger.error("", e);
		} finally {
			try {
